...
# then from within scapy shell:
# execfile("plugins/l2sess-plugin/test/classifier_script.py")

# The rest is in the scapy CLI

# equivalents for ~0 and ~1
minus_one = 4294967295
minus_two = 4294967294
...
ipv4_5tuple_mask_spaces = "000000000000 000000000000 0000 00 00 0000 0000 0000 00 FF 0000 FFFFFFFF FFFFFFFF FFFF FFFF 00000000 00000000 0000 0000 0000 0000"
ipv6_5tuple_mask_spaces = "000000000000 000000000000 0000 0 00 00000 0000 FF 00 FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF FFFF FFFF 00000000 00000000 0000 0000 0000 0000"

# Array of unprocessed masks and values
masks_spaces = [ ipv4_proto_mask_spaces, ipv4_proto_dport_mask_spaces, ipv4_5tuple_mask_spaces, ipv6_proto_mask_spaces, ipv6_proto_dport_mask_spaces, ipv6_5tuple_mask_spaces ]
...
action_new_session = 1 # for output at least
action_acl_match = 4294967295

link_with_prev = [ False, True, True, False, True, True ]
miss_idx = [ action_drop, action_cont, action_cont, action_drop, action_cont, action_cont ]
...
s =  masks_bin_notrailzero_padded[2]
ip4_inbound_session_table = v.classify_add_del_table(True, 0, 32, 20000, n_leading_empty_vectors(s), len(s)/16 - n_leading_empty_vectors(s), minus_one, action_drop, strip_leading_vectors(s, arg_skip))
ip4_inbound_session_table_index = ip4_inbound_session_table.new_table_index

sessions = []
sess_match_index = [ 1, 1, 0, 2, 2, 0 ]
sess_l2_opaque =   [ minus_one, minus_one, 0, minus_one, minus_one, 0 ]
for i in range(0, len(masks_bin_notrailzero_padded)):
  ...
  a_action = sess_match_index[i]
  a_l2_opaque = sess_l2_opaque[i]
  # classify_add_del_session(is_add, table_index, hit_next_index, opaque_index, advance, match, async=False)
  sessions.append(v.classify_add_del_session(True, final_tables[i].new_table_index, a_action, a_l2_opaque, 0, a_match))
...
s1.write("/sbin/ethtool --offload s1_s0 tx off\n")
s2.write("/sbin/ethtool --offload s2_s0 tx off\n")
s1.write("/sbin/ethtool --offload s1_s0 rx off\n")
s2.write("/sbin/ethtool --offload s2_s0 rx off\n")
...
ip4_table_index = final_tables[2].new_table_index
ip6_table_index = final_tables[5].new_table_index
...
# Put ingress session-only stuff
v.classify_set_interface_l2_tables(vpp_if_to_s2.sw_if_index, ip4_inbound_session_table_index, minus_one, minus_one, True)
...
cli("show vlib graph l2s-output-ip6-add")
cli("show vlib graph l2s-output-ip4-add")
