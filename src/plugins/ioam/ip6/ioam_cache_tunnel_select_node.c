	  ...
	  if (IP_PROTOCOL_TCP ==
	          ...
	          (tcp0->flags & TCP_FLAG_ACK) == TCP_FLAG_ACK)
		{
		  ...
		  if (0 == ioam_cache_ts_lookup (ip0,
			...
			if (0 == (result = ioam_cache_ts_update (cache_thread_id,
								 cache_ts_index,
						                 bi0,
								 hbh0)))
			  {
              ...
              if (no_of_responses > 0)
		{
	      ...
	      e2e->pool_index = pool_index0;
              ioam_e2e_id_rewrite_handler((ioam_e2e_id_option_t *)
					  ((u8 *)e2e + sizeof(ioam_e2e_cache_option_t)),
					  b0);
...
void ioam_cache_ts_timer_node_enable (vlib_main_t *vm, u8 enable)
{
  vlib_node_set_state(vm, ioam_cache_ts_timer_tick_node.index,
		      enable == 0 ? VLIB_NODE_STATE_DISABLED:VLIB_NODE_STATE_POLLING);
}
...
expired_cache_ts_timer_callback (u32 *expired_timers)
{
  ...
  vlib_node_increment_counter (cm->vlib_main, ioam_cache_ts_timer_tick_node.index,
                               IOAM_CACHE_TS_TIMER_TICK_ERROR_TIMER,
                               count);
}

static uword
ioam_cache_ts_timer_tick_node_fn (vlib_main_t * vm, vlib_node_runtime_t * node, vlib_frame_t * f)
{
  ioam_cache_main_t *cm = &ioam_cache_main;
