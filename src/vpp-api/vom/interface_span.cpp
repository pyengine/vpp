...
void interface_span::replay()
{
    if (m_config)
    {
        HW::enqueue(new config_cmd(m_config, m_itf_from->handle(), m_itf_to->handle(),
                                   m_state));
    }
}

std::string interface_span::to_string() const
{
    std::ostringstream s;
    s << "Itf Span-config:"
...
void interface_span::update(const interface_span &desired)
{
    if (!m_config)
    {
        HW::enqueue(new config_cmd(m_config, m_itf_from->handle(), m_itf_to->handle(),
...
interface_span::event_handler::event_handler()
{
    ...
    inspect::register_handler({"itf-span"}, "interface span configurations", this);
}

void interface_span::event_handler::handle_replay()
{
    m_db.replay();
}

void interface_span::event_handler::handle_populate(const client_db::key_t &key)
{
    ...
    for (auto & record : *cmd)
    {
        ...
        std::shared_ptr<interface> itf_from = interface::find(payload.sw_if_index_from);
        std::shared_ptr<interface> itf_to = interface::find(payload.sw_if_index_to);

        interface_span itf_span(*itf_from,
                                *itf_to,
                                state_t::from_int(payload.state));

         BOOST_LOG_SEV(logger(), levels::debug) << "span-dump: "
                                                << itf_from->to_string()
                                                << itf_to->to_string()
         ...
         */
        VOM::OM::commit(key, itf_span);
    }

}

dependency_t interface_span::event_handler::order() const
{
    return (dependency_t::BINDING);
}

void interface_span::event_handler::show(std::ostream &os)
{
    m_db.dump(os);
}

const interface_span::state_t interface_span::state_t::DISABLED(0, "disable");
const interface_span::state_t interface_span::state_t::RX_ENABLED(1, "rx-enable");
const interface_span::state_t interface_span::state_t::TX_ENABLED(2, "tx-enable");
const interface_span::state_t interface_span::state_t::TX_RX_ENABLED(3, "tx-rx-enable");
...
interface_span::state_t interface_span::state_t::from_int(uint8_t i)
{
    switch (i) {
        case 0:
