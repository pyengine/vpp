...
class Dot1QSubint(Subint):
    def __init__(self, sub_id, vlan=None):
        if vlan is None:
            vlan = sub_id
        super(Dot1QSubint, self).__init__(sub_id)
        self.vlan = vlan


class Dot1ADSubint(Subint):
    def __init__(self, sub_id, outer_vlan, inner_vlan):
        super(Dot1ADSubint, self).__init__(sub_id)
...
class IPv6Util(Util):
    ...
    def send_neighbour_advertisement_on_software_interfaces(cls, args):
        for i in args:
            ...
            cls.cli(2, "trace add pg-input 1")
            cls.pg_start()
            cls.api('ip_neighbor_add_del pg%i dst %s mac %s' % (i, cls.MY_SOFT_IP6S[i], cls.MY_MACS[i]))


class TestIPv6(IPv6Util, VppTestCase):
    """ IPv6 Test Case """
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            cls.INT_DETAILS[0] = HardInt()

            cls.INT_DETAILS[1] = Dot1QSubint(100)

            cls.api("create_vlan_subif pg%u vlan %u" % (1, cls.INT_DETAILS[1].vlan))
            ...
            # Use dor1q for now
            cls.INT_DETAILS[2] = Dot1QSubint(200)
            cls.api("create_vlan_subif pg%u vlan %u" % (2, cls.INT_DETAILS[2].vlan))

            for i in cls.interfaces:
                det = cls.INT_DETAILS[i]
                if isinstance(det, Subint):
    ...
    def config_ip6_on_software_interfaces(cls, args):
        for i in args:
            ...
            if isinstance(cls.INT_DETAILS[i], Subint):
                interface = "pg%u.%u" % (i, cls.INT_DETAILS[i].sub_id)
            else:
                interface = "pg%u" % i
            cls.api("sw_interface_add_del_address %s %s/64" % (interface, cls.VPP_SOFT_IP6S[i]))
    ...
    def add_dot1_layers(cls, i, packet):
        ...
        det = cls.INT_DETAILS[i]
        if isinstance(det, Dot1QSubint):
            packet.remove_payload()
            packet.add_payload(Dot1Q(vlan=det.sub_id) / payload)
        elif isinstance(det, Dot1ADSubint):
            packet.remove_payload()
    ...
    def create_stream(self, pg_id):
        ...
        for i in range(0, 257):
            ...
            if not isinstance(self.INT_DETAILS[pg_id], Subint):
