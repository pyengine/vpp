                         ...
                         u8 msg_type, u16 ctx)
{
  ...
  trace_len = sizeof (ioam_trace_option_t) +
      (4 * trace_data_size * 2) - sizeof (ip6_hop_by_hop_option_t);
  ...
  trace_option->ioam_trace_type = TRACE_TYPE_IF_TS_APP & TRACE_TYPE_MASK;
  trace_option->data_list_elts_left = 4 * 2;
      //profile->num_elts * 2;
...
udp_ping_populate_flow (ip46_address_t src, ip46_address_t dst,
                        u16 start_src_port, u16 end_src_port,
                        u16 start_dst_port, u16 end_dst_port,
                        u16 interval, ip46_udp_ping_flow *flow)
{
  flow->src = src;
  flow->dst = dst;
  flow->udp_data.start_src_port = start_src_port;
...
udp_ping_send_ip6_pak (vlib_main_t *vm, ip46_udp_ping_flow *flow)
{
  ...
  flow->udp_data.last_sent = vlib_time_now(vm);
}
