...
class VPP():
    ...
    def _register_functions(self):
        ...
        '''
                # Check CRC
                if m[0] not in self.dictionary:
                    logging.warning('Missing API definition for:' + m[0])
                    continue
                if m[-1]['crc'] != '0x' + self.dictionary[m[0]]['crc']:
                    logging.warning('Missing API definition for:' + m[0])
                    continue
                id = self.dictionary[m[0]]['id']
        '''
    ...
    def msg_handler(self, msg):
        ...
        if self.id_names[i] == 'control_ping_reply':
