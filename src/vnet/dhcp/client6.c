...
dhcp6_request_state (dhcp6_client_t * c, f64 now)
{
  if (now >= c->last_state_time + clib_net_to_host_u32(c->opt_ia_pd.t2))
    {
      clib_warning("T2 timeout: Back to discovery state");
...
{
  clib_warning("dhcpv6 state machine for client %d ", pool_index);
  dhcp6_client_main_t * dcm = &dhcp6_client_main;
  dhcp6_client_t * c;
			...
			void **options[DHCPV6_OPTION_MAX])
{
  ...
  while (len)
	...
	{
	  clib_warning("Found option %s (%d): Overflow", dhcpv6_options[opt_id].s, opt_id);
	  return -3;
	}
      else if (opt_len < dhcpv6_options[opt_id].min_size)
	{
	  clib_warning("Found option %s (%d): Too small", dhcpv6_options[opt_id].s, opt_id);
	  return -4;
	}
      else
	{
	  clib_warning("Found option %s (%d)", dhcpv6_options[opt_id].s, opt_id);
					...
					void **options[DHCPV6_OPTION_MAX])
{
  ...
  if (dhcpv6_hdr->msg_type != DHCPV6_MSG_ADVERTISE)
    {
      clib_warning("Ignoring non advertise message while in solicit state");
      return;
    }

  if (!vec_len(options[DHCPV6_OPTION_IA_PD]))
    {
      clib_warning("Ignore message with no PD offer");
      return;
    }
  ...
  if (dhcp6_parse_options((u8 *)(ia + 1),
    ...
    {
      clib_warning("Error parsing DHCP ia pd options");
      return;
    }

  if (vec_len(ia_options[DHCPV6_OPTION_STATUS_CODE]))
    {
      clib_warning("Status code in PD option: %U",
				       ...
				       void **options[DHCPV6_OPTION_MAX])
{
  ...
  if (dhcpv6_hdr->msg_type != DHCPV6_MSG_REPLY)
    {
      clib_warning("Ignoring non reply message while in request state");
      return;
    }

  if (!vec_len(options[DHCPV6_OPTION_IA_PD]))
    {
      clib_warning("Error: Reply with no PD offer");
      return;
    }
  ...
  if (vec_len(ia_options[DHCPV6_OPTION_IAPREFIX]))
    {
      ...
      clib_warning("Obtained lease %U/%d lifetime %u:%u",
				    ...
				    void **options[DHCPV6_OPTION_MAX])
{
  if (dhcpv6_hdr->msg_type != DHCPV6_MSG_REPLY)
    {
      clib_warning("Ignoring non reply message while in bound state");
      return;
    }
...
int dhcp6_client_for_us (vlib_main_t *vm, u32 bi)
{
  ...
  if (b->current_length < sizeof(*dhcpv6_hdr))
    {
      clib_warning("Missing DHCP header");
      return 1;
    }
  ...
  if (dhcp6_parse_options(dhcpv6_hdr->data,
    ...
    {
      clib_warning("Error parsing DHCP main level options");
