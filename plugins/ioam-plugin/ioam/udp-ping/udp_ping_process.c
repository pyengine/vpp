...
vlib_plugin_register (vlib_main_t * vm, vnet_plugin_handoff_t * h,
                      int from_early_init)
{
  udp_ping_main_t *um = &udp_ping_main;
  clib_error_t *error = 0;

  um->vlib_main = vm;
  um->vnet_main = h->vnet_main;
  um->ethernet_main = h->ethernet_main;
  ...
  if (found)
    {
      if (is_disable)
        {
          interval = flow->udp_data.interval;
                         ...
                         vlib_cli_command_t * cmd)
{
  ...
  ip46_udp_ping_set_flow(src, dst, start_src_port, end_src_port,
                        start_dst_port, end_dst_port, (u16)interval, is_disable);
          ...
          for (dst_port = ip46_flow->udp_data.start_dst_port;
              dst_port <= ip46_flow->udp_data.end_dst_port ; dst_port++)
            {
              ...
              s = print_analyse_flow(s,
                                 &ip46_flow->udp_data.stats[j].analyse_data);
              j++;
            }
        }
      s = format(s, "\n\n");
                  ...
                  vlib_node_runtime_t * rt, vlib_frame_t * f)
{
  ...
  while (1)
    {
      f64 next_ping_time;
      next_timeout = 0xFFFFFFFFFFFFFFFF;
      ...
      for (i = 0; i < vec_len(udp_ping_main.ip46_flow); i++)
        {
          ...
          ip46_flow = pool_elt_at_index(udp_ping_main.ip46_flow, i);
          next_ping_time =
              ip46_flow->udp_data.last_sent + ip46_flow->udp_data.interval;
          if (next_ping_time < now)
            {
              udp_ping_send_ip6_pak(udp_ping_main.vlib_main, ip46_flow);
              next_ping_time += ip46_flow->udp_data.interval;
            }
          if (next_timeout > next_ping_time)
            next_timeout = next_ping_time;
        }
      now = vlib_time_now (vm);
#if 0
      if (now < next_timeout)
        interval = 1;//ideally shouldn't happen
      else
        interval = next_timeout - now;
#endif
      interval = 1;
    }
  ...
  while (opt0 < limit0)
    {
      ...
      switch (type0)
      {
        ...
        case HBH_OPTION_TYPE_IOAM_EDGE_TO_EDGE:
          (void)ip6_ioam_analyse_hbh_e2e(data, opt0, len);
          break;
        case 0: /* Pad1 */
      ...
      if ((udp0->ping_data.probe_marker1 ==
              ...
              clib_host_to_net_u32(UDP_PING_PROBE_MARKER2)))
        {
          if (udp0->ping_data.msg_type == UDP_PING_PROBE)
            {
              udp_ping_create_reply_from_probe_ip6(ip0, udp0);
              vnet_buffer(b0)->l2_classify.opaque_index = 0;
              *next0 = UDP_PING_NEXT_IP6_LOOKUP;
            }
          else if (udp0->ping_data.msg_type == UDP_PING_REPLY)
            {
              opt0 = (ip6_hop_by_hop_option_t *) (hbh0 + 1);
