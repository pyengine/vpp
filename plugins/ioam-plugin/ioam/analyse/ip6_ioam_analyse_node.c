...
format_analyse_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "EXPORT: flow_id %d, next index %d",
                     ...
                     vlib_frame_t * frame)
{
  ...
  vlib_node_increment_counter (vm, analyse_node_local.index, ANALYSE_ERROR_ANALYSED,
                               pkts_analysed);

  if (PREDICT_FALSE(pkts_failed))
    vlib_node_increment_counter (vm, analyse_node_local.index, ANALYSE_ERROR_FAILED,
                                 pkts_failed);
