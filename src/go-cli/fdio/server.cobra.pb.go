...
var _DefaultInterfaceClientCommandConfig = _NewInterfaceClientCommandConfig()

type _InterfaceClientCommandConfig struct {
...
func _NewInterfaceClientCommandConfig() *_InterfaceClientCommandConfig {
	c := &_InterfaceClientCommandConfig{}
	envconfig.Process("", c)
	return c
}

func (o *_InterfaceClientCommandConfig) AddFlags(fs *pflag.FlagSet) {
...
var InterfaceClientCommand = &cobra.Command{
	Use: "interface",
}

func _DialInterface() (*grpc.ClientConn, InterfaceClient, error) {
	cfg := _DefaultInterfaceClientCommandConfig
	...
	return conn, NewInterfaceClient(conn), nil
}

type _InterfaceRoundTripFunc func(cli InterfaceClient, in iocodec.Decoder, out iocodec.Encoder) error

func _InterfaceRoundTrip(sample interface{}, fn _InterfaceRoundTripFunc) error {
	cfg := _DefaultInterfaceClientCommandConfig
	...
	conn, client, err := _DialInterface()
	if err != nil {
		return err
	}
	defer conn.Close()
...
Authenticate using the Authorization header (requires transport security):
	...
	echo '{json}' | createloopback --tls`,
	Run: func(cmd *cobra.Command, args []string) {
		var v CvlApiCreateLoopbackT
		err := _InterfaceRoundTrip(v, func(cli InterfaceClient, in iocodec.Decoder, out iocodec.Encoder) error {
			...
			resp, err := cli.CreateLoopback(context.Background(), &v)
...
func init() {
	InterfaceClientCommand.AddCommand(_InterfaceCreateLoopbackClientCommand)
	_DefaultInterfaceClientCommandConfig.AddFlags(_InterfaceCreateLoopbackClientCommand.Flags())
}
