		    ...
		    vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  while (n_left_from > 0)
	...
	{
	  u32 next0 = CACHE_NEXT_POP_HBYH;
	  u32 next1 = CACHE_NEXT_POP_HBYH;
	  u32 bi0, bi1;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  /* Prefetch next iteration. */
	  {
	    ...
	    CLIB_PREFETCH (p2->data - rewrite_len,
			   2 * CLIB_CACHE_LINE_BYTES, STORE);
	    CLIB_PREFETCH (p3->data - rewrite_len,
			   2 * CLIB_CACHE_LINE_BYTES, STORE);
	  }
	  ...
	  copy_dst1 = (u64 *) (((u8 *) ip1) - rewrite_len);
	  copy_src0 = (u64 *) ip0;
	  copy_src1 = (u64 *) ip1;
	  ...
	  vlib_buffer_advance (b1, -(word) rewrite_len);
	  ip0 = vlib_buffer_get_current (b0);
	  ip1 = vlib_buffer_get_current (b1);
	  ...
	  clib_memcpy (hbh1, rewrite, rewrite_len);
	  ...
	  new_l1 =
	    clib_net_to_host_u16 (ip1->payload_length) + rewrite_len;
